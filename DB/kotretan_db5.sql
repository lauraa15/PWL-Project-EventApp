...
CREATE TABLE IF NOT EXISTS `eventsapp`.`event_sessions` (
  ...
  `registration_fee` DECIMAL(10,2) NULL DEFAULT '0.00' COMMENT 'Fee khusus untuk sesi ini (jika registration_type = session_only atau both)',
  `max_participants` INT NULL DEFAULT NULL COMMENT 'Jika NULL, ikuti max dari event',
  `current_participants` INT NULL DEFAULT '0',
  `is_mandatory` BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Apakah sesi ini wajib dihadiri (untuk registration_type = event_only)',
  `allow_individual_registration` BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Boleh daftar sesi ini secara terpisah',
  `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `eventsapp`.`payments` (
  ...
  `verified_by` INT NULL COMMENT 'User ID yang memverifikasi (tim keuangan)',
  ...
  INDEX `fk_payments_verified_by_idx` (`verified_by` ASC),
...
CREATE TABLE IF NOT EXISTS `eventsapp`.`attendances` (
  ...
  `scanned_by` INT NOT NULL COMMENT 'User ID panitia yang melakukan scan',
  ...
  `certificate_issued_by` INT NULL COMMENT 'User ID panitia yang menerbitkan sertifikat',
  `notes` TEXT NULL,
  `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  INDEX `fk_attendances_scanned_by_idx` (`scanned_by` ASC),
  INDEX `fk_attendances_cert_issued_by_idx` (`certificate_issued_by` ASC),
  ...
  CONSTRAINT `fk_attendances_sessions`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_attendances_scanned_by`
    FOREIGN KEY (`scanned_by`)
    REFERENCES `eventsapp`.`users` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_attendances_cert_issued_by`
    FOREIGN KEY (`certificate_issued_by`)
    REFERENCES `eventsapp`.`users` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE)
ENGINE = InnoDB
